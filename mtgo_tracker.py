...
import csv
import modo
import mtg_goldfish_lists
import multifaced_cards
import os
import time
import io
import datetime
...
def save_window(exit):
    ...
    if exit:
        ...
        button_nosave = tk.Button(bot_frame, text="Don't Save", width=10,
                                  command=lambda: [close_save_window(), close()])
        button_save = tk.Button(bot_frame, text="Save", width=10,
                                command=lambda: [close_save_window(), save(exit=True)])
        button_nosave.grid(row=0, column=1, padx=5, pady=5)
    else:
        t = "This will overwrite any previously saved data.\nAre you sure you want to save?"
        button_save = tk.Button(bot_frame, text="Save", width=10,
                                command=lambda: [close_save_window(), save(exit=False)])
...
def delete_session():
    ...
    label1 = tk.Label(mid_frame,
                      text="This will delete your previous saved session.\nAre you sure you want to continue?",
                      wraplength=width)
...
def save_settings():
    os.chdir(FILEPATH_ROOT + "\\" + "save")
    SETTINGS = [FILEPATH_ROOT, FILEPATH_EXPORT, FILEPATH_LOGS, FILEPATH_LOGS_COPY, FILEPATH_DRAFTS,
                FILEPATH_DRAFTS_COPY, HERO]
                ...
                if ("Match_GameLog_" not in i) or (len(i) < 30):
                    ...
                    if copy:
                        try:
                            ...
                            os.chdir(FILEPATH_LOGS_COPY)
                            os.utime(i, (datetime.datetime.now().timestamp(),
                                         datetime.datetime.strptime(mtime, "%a %b %d %H:%M:%S %Y").timestamp()))
                            os.chdir(root)
...
def print_data(data, headers, update_status, start_index, apply_filter):
    ...
    small_headers = ["P1_Roll", "P2_Roll", "P1_Wins", "P2_Wins", "Game_Num", "Play_Num", "Turn_Num", "Pack_Num",
                     "Pick_Num", "Pick_Ovr"]
    med_headers = ["Avail_1", "Avail_2", "Avail_3", "Avail_4", "Avail_5", "Avail_6", "Avail_7", "Avail_8", "Avail_9",
                   "Avail_10", "Avail_11", "Avail_12", "Avail_13", "Avail_14"]
    large_headers = ["Card"]
    ...
    if new_import == True:
        ...
        update_status_bar(
            status=f"Displaying: {str(start_index + 1)}-{str(end_index)} of {str(len(df_rows))} total records.")
...
def input_missing_data():
    ...
    for i in ALL_DATA[0]:  # Iterate through matches.
        ...
        if (i[p1_arch_index] == "NA") or (i[p1_sub_index] == "NA") or \
            ...
            cards1 = df[(df.Casting_Player == players[0]) & (
                        df.Action == "Land Drop")].Primary_Card.value_counts().keys().tolist()
            cards2 = df[
                (df.Casting_Player == players[0]) & (df.Action == "Casts")].Primary_Card.value_counts().keys().tolist()
            cards3 = df[(df.Casting_Player == players[1]) & (
                        df.Action == "Land Drop")].Primary_Card.value_counts().keys().tolist()
            cards4 = df[
                (df.Casting_Player == players[1]) & (df.Action == "Casts")].Primary_Card.value_counts().keys().tolist()
    ...
    for i in ALL_DATA[0]:
        ...
        if update_type == "Limited":
            if i[format_index] in INPUT_OPTIONS["Limited Formats"]:
                ask_to_save = True
                cards1 = df2[(df2.Casting_Player == players[0]) & (
                            df2.Match_ID == i[0])].Primary_Card.value_counts().keys().tolist()
                cards2 = df2[(df2.Casting_Player == players[1]) & (
                            df2.Match_ID == i[0])].Primary_Card.value_counts().keys().tolist()
        ...
        # Update P1_Subarch, P2_Subarch for all Matches.
            ...
            ask_to_save = True
            cards1 = df2[
                (df2.Casting_Player == players[0]) & (df2.Match_ID == i[0])].Primary_Card.value_counts().keys().tolist()
            cards2 = df2[
                (df2.Casting_Player == players[1]) & (df2.Match_ID == i[0])].Primary_Card.value_counts().keys().tolist()
            if i[format_index] in INPUT_OPTIONS["Limited Formats"]:
                i[p1_sa_index] = modo.get_limited_subarch(cards1)
                i[p2_sa_index] = modo.get_limited_subarch(cards2)
            else:
...
def rerun_decks_window():
    ...
    def guess(mode):
        if mode == "NA/Unknown":
            t = "Updated the P1_Subarch, P2_Subarch columns for Unknown Decks in the Date Range: " + \
                list(ALL_DECKS.keys())[0]
            deck_data_guess(update_type="Unknowns")
        elif mode == "Limited":
            t = "Updated the P1_Subarch, P2_Subarch columns for Limited Matches in the Date Range: " + \
                list(ALL_DECKS.keys())[0]
            deck_data_guess(update_type="Limited")
        elif mode == "Constructed":
            t = "Updated the P1_Subarch, P2_Subarch columns for Constructed Matches in the Date Range: " + \
                list(ALL_DECKS.keys())[0]
            deck_data_guess(update_type="Constructed")
        elif mode == "Overwrite All":
            t = "Updated the P1_Subarch, P2_Subarch columns for each Match in the Date Range: " + \
                list(ALL_DECKS.keys())[0]
    ...
    def import_decks():
        ...
        if len(ALL_DECKS) == 0:
            ...
            label2["text"] = "Sample decklists loaded: " + list(ALL_DECKS.keys())[0] + " to " + list(ALL_DECKS.keys())[
                -1]
            button_apply["state"] = tk.NORMAL
    ...
    label3 = tk.Label(mid_frame,
                      text="This will apply best guess deck names in the P1/P2_Subarch columns.\n\nChoose which rows to apply changes.",
                      wraplength=width)
...
def revise_entry_window(players, cards1, cards2, card3, cards4, progress, mdata):
    def close_format_window(*argv):
        ...
        if len(argv) > 0:
            ...
            missing_data = [p1_arch.get(), p1_sub.get().strip(), p2_arch.get(), p2_sub.get().strip(), mformat.get(),
                            dformat.get(), mtype.get()]
    ...
    def update_arch(*argv):
        ...
        menu.delete(0, "end")
        if mformat.get() == "NA":
            l = ["NA"] + INPUT_OPTIONS["Constructed Match Types"] + INPUT_OPTIONS["Booster Draft Match Types"] + \
                INPUT_OPTIONS["Sealed Match Types"]
            for i in l:
        ...
        if mformat.get() in INPUT_OPTIONS["Limited Formats"]:
            ...
            if (mdata[modo.header("Matches").index("P1_Arch")] == "NA") or (
                    mdata[modo.header("Matches").index("P1_Arch")] == "Limited"):
                p1_arch.set("P1 Archetype")
            else:
                p1_arch.set(mdata[modo.header("Matches").index("P1_Arch")])
            if (mdata[modo.header("Matches").index("P2_Arch")] == "NA") or (
                    mdata[modo.header("Matches").index("P2_Arch")] == "Limited"):
    ...
    if mformat.get() == "Cube":
        ...
        type_options += INPUT_OPTIONS["Constructed Match Types"] + INPUT_OPTIONS["Booster Draft Match Types"] + \
                        INPUT_OPTIONS["Sealed Match Types"]
...
def back():
    ...
    if (display == "Drafts") or (display == "Picks"):
        print_data(curr_data, headers=modo.header(display), update_status=True, start_index=display_index,
                   apply_filter=False)
    else:
        print_data(curr_data, headers=modo.header(display), update_status=True, start_index=display_index,
                   apply_filter=False)
...
def export2(current=False, matches=False, games=False, plays=False, drafts=False, picks=False, _csv=False, _excel=False,
    ...
    if _csv:
        ...
        try:
            ...
            update_status_bar(
                status="Permission Error: Common error cause is an open file that can not be overwritten.")
    elif _excel:
        for index, i in enumerate(file_names):
...
def sort_column2(col, reverse, tree1):
    ...
    if display == "Matches":
        ...
        curr_data.sort_values(by=[col, "Match_ID", "Game_Num"], inplace=True, ascending=(reverse, False, True),
                              key=sort_key)
    elif display == "Plays":
        curr_data.sort_values(by=[col, "Match_ID", "Game_Num", "Play_Num"], inplace=True,
                              ascending=(reverse, False, True, True), key=sort_key)
    elif display == "Drafts":
        curr_data.sort_values(by=[col, "Draft_ID"], inplace=True, ascending=(reverse, False), key=sort_key)
    elif display == "Picks":
        curr_data.sort_values(by=[col, "Draft_ID", "Pick_Ovr"], inplace=True, ascending=(reverse, False, True),
                              key=sort_key)
...
def revise_record2():
    ...
    cards1 = df[
        (df.Casting_Player == players[0]) & (df.Action == "Land Drop")].Primary_Card.value_counts().keys().tolist()
    cards2 = df[(df.Casting_Player == players[0]) & (df.Action == "Casts")].Primary_Card.value_counts().keys().tolist()
    cards3 = df[
        (df.Casting_Player == players[1]) & (df.Action == "Land Drop")].Primary_Card.value_counts().keys().tolist()
...
def revise_record():
    ...
    def update_arch(*argv):
        ...
        if match_format.get() == "NA":
            for i in ["NA"] + INPUT_OPTIONS["Constructed Match Types"] + INPUT_OPTIONS["Booster Draft Match Types"] + \
                     INPUT_OPTIONS["Sealed Match Types"]:
    ...
    if match_format.get() == "Cube":
        ...
        limited_options = ["NA"]
        match_options = ["NA"] + INPUT_OPTIONS["Constructed Match Types"] + INPUT_OPTIONS["Booster Draft Match Types"] + \
                        INPUT_OPTIONS["Sealed Match Types"]
...
def revise_record_multi():
    ...
    elif (sel_formats["constructed"] == False) and (sel_formats["booster"] == True) and (
            sel_formats["sealed"] == False):
        match_options += INPUT_OPTIONS["Booster Draft Match Types"]
    elif (sel_formats["constructed"] == False) and (sel_formats["booster"] == False) and (
            sel_formats["sealed"] == True):
        match_options += INPUT_OPTIONS["Sealed Match Types"]
    elif (sel_formats["constructed"] == False) and (sel_formats["booster"] == False) and (
            sel_formats["sealed"] == False):
        match_options += INPUT_OPTIONS["Constructed Match Types"] + INPUT_OPTIONS["Booster Draft Match Types"] + \
                         INPUT_OPTIONS["Sealed Match Types"]
    match_type = tk.StringVar()
        ...
        if overwrite == True:
            ...
            for i in ALL_DATA[0]:
                try:
                    ...
                    i[modo.header("Matches").index("P1_Arch")] = \
                    match_dict[i[0]][0][i[modo.header("Matches").index("P1")]][0]
                    i[modo.header("Matches").index("P1_Subarch")] = \
                    match_dict[i[0]][0][i[modo.header("Matches").index("P1")]][1]
                    i[modo.header("Matches").index("P2_Arch")] = \
                    match_dict[i[0]][0][i[modo.header("Matches").index("P2")]][0]
                    i[modo.header("Matches").index("P2_Subarch")] = \
                    match_dict[i[0]][0][i[modo.header("Matches").index("P2")]][1]
            ...
            for i in DRAFTS_TABLE:
                wins = df_inverted[
                    (df_inverted.Draft_ID == i[0]) & (df_inverted.P1 == i[modo.header("Drafts").index("Hero")]) & (
                                df_inverted.Match_Winner == "P1")].shape[0]
                losses = df_inverted[
                    (df_inverted.Draft_ID == i[0]) & (df_inverted.P1 == i[modo.header("Drafts").index("Hero")]) & (
                                df_inverted.Match_Winner == "P2")].shape[0]
...
def get_winners():
    ...
    if len(ALL_DATA[3]) > len(raw_dict_new):
        ...
        for i in DRAFTS_TABLE:
            wins = df_inverted[
                (df_inverted.Draft_ID == i[0]) & (df_inverted.P1 == i[1]) & (df_inverted.Match_Winner == "P1")].shape[0]
            losses = df_inverted[
                (df_inverted.Draft_ID == i[0]) & (df_inverted.P1 == i[1]) & (df_inverted.Match_Winner == "P2")].shape[0]
            i[modo.header("Drafts").index("Match_Wins")] = wins
            i[modo.header("Drafts").index("Match_Losses")] = losses
...
def ask_for_winner(ga_list, p1, p2, n, total):
    ...
    label2 = tk.Label(mid_frame, text=all_ga.replace('@[', '').replace('@]', ''), anchor="center", wraplength=width,
                      justify="left")
...
def get_stats():
    ...
    def match_stats(hero, opp, mformat, lformat, deck, opp_deck, date_range, s_type):
        ...
        if (format_wins[0] + format_losses[0]) == 0:
            ...
            format_wr = [
                to_percent(format_wins[0] / (format_wins[0] + format_losses[0]), 1) + "%"]  # adding overall in L[0]
        ...
        for i in matchtypes_played:
            if mformat != "All Formats":
                mt_wins = \
                df0_i_f[(df0_i_f.Format == mformat) & (df0_i_f.Match_Type == i) & (df0_i_f.Match_Winner == "P1")].shape[
                    0]
                mt_losses = \
                df0_i_f[(df0_i_f.Format == mformat) & (df0_i_f.Match_Type == i) & (df0_i_f.Match_Winner == "P2")].shape[
                    0]
            else:
        ...
        roll_values = [roll_1_mean, roll_2_mean, p1_roll_wr + "%", p2_roll_wr + "%", "",
                       to_percent(rolls_won / hero_n, 1) + "%"]
        ...
        for i in range(10):
            ...
            if tagged == True:
                tree3.insert("", "end", values=[hero_decks[i],
                                                (str(hero_deck_counts[i]) + " - (" + to_percent(
                                                    hero_deck_counts[i] / filtered_n, 0) + "%)"),
                                                hero_deck_wr[i][0],
                                                hero_deck_wr[i][1],
        ...
        for i in range(10):
            ...
            if tagged == True:
                tree4.insert("", "end", values=[meta_decks[i],
                                                (str(meta_deck_counts[i]) + " - (" + to_percent(
                                                    meta_deck_counts[i] / filtered_n, 0) + "%)"),
                                                meta_deck_wr[i][0],
                                                meta_deck_wr[i][1],
        ...
        if (deck != "All Decks") & (opp_deck != "All Opp. Decks"):
            ...
            for i in df_list:
                ...
                if total_n == 0:
                    ...
                    hero_mull_rate = round(
                        (i[(i.P1_Subarch == deck) & (i.P2_Subarch == opp_deck)].P1_Mulls.sum() / total_n), 2)
                    opp_mull_rate = round(
                        (i[(i.P1_Subarch == deck) & (i.P2_Subarch == opp_deck)].P2_Mulls.sum() / total_n), 2)
    ...
    def play_stats(hero, opp, mformat, lformat, deck, opp_deck, date_range, s_type):
        ...
        for i in hero_decks:
            if mformat != "All Formats":
                games_played = df1_i_merge[
                    (df1_i_merge.P1 == hero) & (df1_i_merge.Format == mformat) & (df1_i_merge.P1_Subarch == i)].shape[0]
            else:
                games_played = df1_i_merge[(df1_i_merge.P1 == hero) & (df1_i_merge.P1_Subarch == i)].shape[0]
        ...
        for i in opp_decks:
            if mformat != "All Formats":
                games_played = df1_i_merge[
                    (df1_i_merge.P1 == hero) & (df1_i_merge.Format == mformat) & (df1_i_merge.P2_Subarch == i)].shape[0]
            else:
                games_played = df1_i_merge[(df1_i_merge.P1 == hero) & (df1_i_merge.P2_Subarch == i)].shape[0]
    ...
    def card_stats(hero, opp, mformat, lformat, deck, opp_deck, date_range, s_type):
        ...
        df_merge = pd.merge(df0_i,
                            ...
                            right_on=["Match_ID", "P1", "P2"])
        df_merge = df_merge[
            (df_merge.Game_Winner != "NA") & (df_merge.Date > date_range[0]) & (df_merge.Date < date_range[1])]
        if mformat != "All Formats":
        ...
        df_merge_pre = df_merge_pre[
            (df_merge_pre.Casting_Player == hero) & (df_merge_pre.Action.isin(["Plays", "Casts"]))]
        df_merge_pre.drop(df_merge_pre.columns.difference(
            ["Game_ID", "Game_Num", "P1_Subarch", "P2_Subarch", "Primary_Card", "Won_Game"]), axis=1, inplace=True)
        df_merge_pre.drop_duplicates(inplace=True)

        df_merge_post = df_merge_post[
            (df_merge_post.Casting_Player == hero) & (df_merge_post.Action.isin(["Plays", "Casts"]))]
        df_merge_post.drop(df_merge_post.columns.difference(
            ["Game_ID", "Game_Num", "P1_Subarch", "P2_Subarch", "Primary_Card", "Won_Game"]), axis=1, inplace=True)
    ...
    def update_lim_menu(*argv):
        lim_formats_played = df0_i[
            (df0_i.P1 == player.get()) & (df0_i.Format == mformat.get())].Limited_Format.value_counts().keys().tolist()
    ...
    def update_deck_menu(*argv):
        if mformat.get() == "All Formats":
            ...
            df = df0_i[(df0_i.P1 == player.get()) & (df0_i.Format == mformat.get()) & (
                        df0_i.Limited_Format == lim_format.get())]
        else:
            df = df0_i[(df0_i.P1 == player.get()) & (df0_i.Format == mformat.get())]
    ...
    def load_data(*argv):
        ...
        if date_entry_1.get() < date_entry_2.get():
            ...
            dr = [date_entry_1.get() + "-00:00", date_entry_2.get() + "-23:59"]
        if s_type.get() == "Match History":
            match_history(player.get(), opponent.get(), mformat.get(), lim_format.get(), deck.get(), opp_deck.get(), dr,
                          s_type.get())
        elif s_type.get() == "Match Stats":
            match_stats(player.get(), opponent.get(), mformat.get(), lim_format.get(), deck.get(), opp_deck.get(), dr,
                        s_type.get())
        elif s_type.get() == "Game Stats":
            game_stats(player.get(), opponent.get(), mformat.get(), lim_format.get(), deck.get(), opp_deck.get(), dr,
                       s_type.get())
        elif s_type.get() == "Play Stats":
            play_stats(player.get(), opponent.get(), mformat.get(), lim_format.get(), deck.get(), opp_deck.get(), dr,
                       s_type.get())
        elif s_type.get() == "Opponent Stats":
            opp_stats(player.get(), opponent.get(), mformat.get(), lim_format.get(), deck.get(), opp_deck.get(), dr,
                      s_type.get())
        elif s_type.get() == "Time Data":
            time_stats(player.get(), opponent.get(), mformat.get(), lim_format.get(), deck.get(), opp_deck.get(), dr,
                       s_type.get())
        elif s_type.get() == "Card Data":
            card_stats(player.get(), opponent.get(), mformat.get(), lim_format.get(), deck.get(), opp_deck.get(), dr,
                       s_type.get())

        status_text = "Loaded Data:" + player.get() + "," + opponent.get() + "," + mformat.get() + "," + lim_format.get() + "," + deck.get() + "," + opp_deck.get() + "," + \
                      dr[0] + "," + dr[1] + "," + s_type.get()
        print(status_text)
...
def remove_select():
    ...
    button_delete = tk.Button(bot_frame, text="Remove", width=10,
                              command=lambda: [close_window(), remove_record(ignore=False)])
    button_ignore = tk.Button(bot_frame, text="Remove and Ignore", width=15,
                              command=lambda: [close_window(), remove_record(ignore=True)])
    button_close = tk.Button(bot_frame, text="Cancel", width=10, command=lambda: close_window())
...
def get_associated_draftid(mode):
    ...
    df_matches = df_matches[
        df_matches.Limited_Format.isin(INPUT_OPTIONS["Cube Formats"] + INPUT_OPTIONS["Booster Draft Formats"])]
    if mode == "NA":
    ...
    if len(list_to_process) > 0:
        ...
        df_inverted = pd.DataFrame(ALL_DATA_INVERTED[0], columns=modo.header("Matches"))
        for i in DRAFTS_TABLE:
            wins = df_inverted[(df_inverted.Draft_ID == i[0]) & (df_inverted.P1 == i[hero_index]) & (
                        df_inverted.Match_Winner == "P1")].shape[0]
            losses = df_inverted[(df_inverted.Draft_ID == i[0]) & (df_inverted.P1 == i[hero_index]) & (
                        df_inverted.Match_Winner == "P2")].shape[0]
            i[modo.header("Drafts").index("Match_Wins")] = wins
            i[modo.header("Drafts").index("Match_Losses")] = losses
...
def get_associated_draftid_pre():
    ...
    button_all = tk.Button(bot_frame, text="All", width=10,
                           command=lambda: [close_sub_window(), get_associated_draftid(mode="All")])
    button_na = tk.Button(bot_frame, text="Only 'NA'", width=10,
                          command=lambda: [close_sub_window(), get_associated_draftid(mode="NA")])
...
def update_deck_data_in_prog_window(first_month, last_month):
    global scraper_running
    height = 200
    width = 400
    update_deck_data_in_prog_window = tk.Toplevel(window)
    update_deck_data_in_prog_window.title("MTGGoldfish Deck Data Scraper")
    update_deck_data_in_prog_window.iconbitmap(update_deck_data_in_prog_window, "icon.ico")
    update_deck_data_in_prog_window.minsize(width, height)
    update_deck_data_in_prog_window.resizable(False, False)
    update_deck_data_in_prog_window.focus()
    update_deck_data_in_prog_window.geometry("+%d+%d" %
                                             (window.winfo_x() + (window.winfo_width() / 2) - (width / 2),
                                              window.winfo_y() + (window.winfo_height() / 2) - (height / 2)))

    thread_queue = queue.Queue()

    # Find path of original file to remove later
    original_file = None
    for file in os.listdir(os.getcwd()):
        if file.startswith('ALL_DECKS'):
            original_file = file
    ...
    def process_queue():
        if not thread_queue.empty():
            label3["text"] = f"Progress: {(len(formats) * len(months)) - thread_queue.qsize()} / {(len(formats) * len(months))}"
            update_deck_data_in_prog_window.after(10000, process_queue)
        else:
            done_scraping()
    ...
    def done_scraping():
        ...
        with open(f'ALL_DECKS_{last_month}', "rb") as deck_dump:
            new_decks = pickle.load(deck_dump)
            ALL_DECKS.update(new_decks)

        pickle.dump(ALL_DECKS, open(f'ALL_DECKS_{last_month}', "wb"))
        if original_file is not None:
            os.remove(original_file)

        label1["text"] = "Update complete! Feel free to close the window. For best results, restart MTGO-Tracker now."
        label2["text"] = ""
        label3["text"] = ""
        scraper_running = False
        data_menu.entryconfig("Scrape MTGGoldfish for Decklist Data", state=tk.NORMAL)

    def close_window():
        if not scraper_running:
            update_deck_data_in_prog_window.grab_release()
            update_deck_data_in_prog_window.destroy()

    mid_frame = tk.LabelFrame(update_deck_data_in_prog_window)
    bot_frame = tk.Frame(update_deck_data_in_prog_window)
    mid_frame.grid(row=0, column=0, sticky="nsew")
    bot_frame.grid(row=1, column=0, sticky="")

    update_deck_data_in_prog_window.grid_columnconfigure(0, weight=1)
    update_deck_data_in_prog_window.rowconfigure(0, weight=1)
    mid_frame.grid_columnconfigure(0, weight=1)
    bot_frame.grid_columnconfigure(0, weight=1)
    bot_frame.grid_rowconfigure(0, weight=1)
    bot_frame.grid_rowconfigure(1, weight=1)

    label1 = tk.Label(mid_frame, text="Scrape in progress, please be patient while the data is collected.\n",
                      wraplength=width)
    label2 = tk.Label(mid_frame, text="")
    label3 = tk.Label(mid_frame, text="Progress: ")

    label1.grid(row=1, column=0, padx=10, pady=(20, 0), sticky="nsew")
    label2.grid(row=2, column=0, padx=10, pady=(20, 0), sticky="nsew")
    label3.grid(row=3, column=0, padx=10, pady=(20,0), sticky="nsew")

    months = []
    formats = ["legacy", "modern", "pauper", "pioneer", "standard", "vintage"]

    current = [int(first_month.split("-")[0]), int(first_month.split("-")[1])]
    end = [int(last_month.split("-")[0]), int(last_month.split("-")[1])]
    ...
    while current != end:
        ...
        if current[1] > 12:
            ...
            current[0] += 1
        months.append(str(current[0]) + "-" + f"{current[1]:02}")

    label2["text"] = "Estimated Wait Time: " + str(len(formats) * 5 * len(months)) + " Minutes."

    multifaced_cards.scrape_mfdcs()
    for i in range(len(formats) * len(months)):
        thread_queue.put(i)
    scrape_thread = threading.Thread(target=mtg_goldfish_lists.scrape_lists, args=(thread_queue, months, formats), daemon=True)
    scrape_thread.start()
    scraper_running = True
    data_menu.entryconfig("Scrape MTGGoldfish for Decklist Data", state=tk.DISABLED)
    process_queue()

    update_deck_data_in_prog_window.protocol("WM_DELETE_WINDOW", lambda: close_window())


def update_deck_data_window():
    height = 200
    width = 400
    update_deck_data_window = tk.Toplevel(window)
    update_deck_data_window.title("MTGGoldfish Deck Data Scraper")
    update_deck_data_window.iconbitmap(update_deck_data_window, "icon.ico")
    update_deck_data_window.minsize(width, height)
    update_deck_data_window.resizable(False, False)
    update_deck_data_window.grab_set()
    update_deck_data_window.focus()
    update_deck_data_window.geometry("+%d+%d" %
                                     (window.winfo_x() + (window.winfo_width() / 2) - (width / 2),
                                      window.winfo_y() + (window.winfo_height() / 2) - (height / 2)))

    def scrape_goldfish():
        first_month = (datetime.date.today() + dateutil.relativedelta.relativedelta(months=-1)).strftime("%Y-%m")
        last_month = datetime.date.today().strftime("%Y-%m")
        if len(ALL_DECKS) != 0:
            first_month = list(ALL_DECKS.keys())[-1]

        close_window()
        update_deck_data_in_prog_window(first_month, last_month)
        return

    def close_window():
        update_deck_data_window.grab_release()
        update_deck_data_window.destroy()

    mid_frame = tk.LabelFrame(update_deck_data_window)
    bot_frame = tk.Frame(update_deck_data_window)
    mid_frame.grid(row=0, column=0, sticky="nsew")
    bot_frame.grid(row=1, column=0, sticky="")

    update_deck_data_window.grid_columnconfigure(0, weight=1)
    update_deck_data_window.rowconfigure(0, weight=1)
    mid_frame.grid_columnconfigure(0, weight=1)
    bot_frame.grid_columnconfigure(0, weight=1)
    bot_frame.grid_rowconfigure(0, weight=1)
    bot_frame.grid_rowconfigure(1, weight=1)

    button1 = tk.Button(mid_frame, text="Scrape MTGGoldfish Data", width=20, command=lambda: scrape_goldfish())
    label1 = tk.Label(mid_frame, text="")
    label2 = tk.Label(mid_frame,
                      text="This will scrape MTGGoldfish for format data up to " +
                           datetime.date.today().strftime("%Y-%m") +
                           " for Legacy, Modern, Pauper, Pioneer, Standard, and Vintage. Old format data will not be" +
                           " overriden.\n\nNote this process takes 5 min per month per format and therefore the " +
                           "update may take a long time.",
                      wraplength=width)
    button_close = tk.Button(bot_frame, text="Cancel", width=10, command=lambda: close_window())

    if len(ALL_DECKS) == 0:
        label1["text"] = "Last month currently loaded: NONE, scrape will download latest two months as default."
    elif list(ALL_DECKS.keys())[-1] == datetime.date.today().strftime("%Y-%m"):
        label1["text"] = "Already up to date!"
        button1["state"] = tk.DISABLED
    else:
        label1["text"] = "Last month currently loaded: " + list(ALL_DECKS.keys())[-1]

    label1.grid(row=1, column=0, padx=10, pady=(20, 0), sticky="nsew")
    button1.grid(row=2, column=0, padx=10, pady=5)
    label2.grid(row=3, column=0, padx=10, pady=(10, 5), sticky="nsew")
    button_close.grid(row=0, column=2, padx=10, pady=10)

    update_deck_data_window.protocol("WM_DELETE_WINDOW", lambda: close_window())
...
export_csv.add_command(label="All Data (5 Files)", \
                       command=lambda: export2(matches=True, games=True, plays=True, drafts=True, picks=True,
                                               _csv=True))
export_csv.add_separator()
export_csv.add_command(label="Match History (Inverse Join)",
                       command=lambda: export2(matches=True, _csv=True, inverted=True))
export_csv.add_command(label="Game History (Inverse Join)",
                       command=lambda: export2(games=True, _csv=True, inverted=True))
export_csv.add_command(label="All Data (Inverse Join, 5 Files)", \
                       command=lambda: export2(matches=True, games=True, plays=True, drafts=True, picks=True, _csv=True,
                                               inverted=True))
export_csv.add_separator()
export_csv.add_command(label="Currently Displayed Data (with Filters)",
                       command=lambda: export2(current=True, _csv=True, filtered=True))
...
export_excel.add_command(label="Match History (Inverse Join)",
                         command=lambda: export2(matches=True, _excel=True, inverted=True))
export_excel.add_command(label="Game History (Inverse Join)",
                         command=lambda: export2(games=True, _excel=True, inverted=True))
export_excel.add_command(label="All Data (Inverse Join, 5 Files)", \
                         command=lambda: export2(matches=True, games=True, plays=True, drafts=True, picks=True,
                                                 _excel=True, inverted=True))
export_excel.add_separator()
export_excel.add_command(label="Currently Displayed Table (with Filters)",
                         command=lambda: export2(current=True, _excel=True, filtered=True))
...
data_menu.add_command(label="Apply Associated Draft_IDs to Limited Matches",
                      command=lambda: get_associated_draftid_pre(), state=tk.DISABLED)
data_menu.add_separator()
...
data_menu.add_separator()
data_menu.add_command(label="Scrape MTGGoldfish for Decklist Data", command=lambda: update_deck_data_window())
